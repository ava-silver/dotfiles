...
import type { AgentSession, AgentSessionEventListener, ToolDefinition } from "@earendil-works/pi-coding-agent";
...
test("first assistant response disarms the watchdog without limiting the run", async () => {
	...
	const result = await watchdog.waitFor(new Promise<string>((resolve) => setTimeout(() => resolve("done"), 20)));
