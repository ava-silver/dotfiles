...
const createStubRuntime = () => ManagedRuntime.make(makeSubagentManagerLayer(stubSpawn));
...
async function withManager(
	run: (manager: SubagentManagerShape, runtime: ReturnType<typeof createStubRuntime>) => Promise<void>,
) {
...
test("FAIL: prompts settle as errors; unconsumed settles are delivered", async () => {
	await withManager(async (manager, runtime) => {
		...
		const snap = await runTool(runtime, manager.spawn(task("FAIL: blow up please")));
...
test("the concurrency cap rejects a subagent over the limit", async () => {
	await withManager(async (manager, runtime) => {
		...
		await assert.rejects(runTool(runtime, manager.spawn(task(`Task ${MAX_RUNNING + 1}`))), /Max \d+ subagents/);
	});
});
...
test("pi spawn fails fast without the parent model registry", async () => {
	...
	try {
		const manager = await runtime.runPromise(SubagentManager);
		await assert.rejects(runTool(runtime, manager.spawn(task("needs a registry"))), /model registry/);
...
test("idle restarts respect the concurrency cap", async () => {
	await withManager(async (manager, runtime) => {
		...
		await assert.rejects(runTool(runtime, manager.send(settled.id, "go again")), /Max \d+ subagents/);
