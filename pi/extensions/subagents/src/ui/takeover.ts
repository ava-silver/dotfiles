...
export async function openSubagentPicker(ctx: ExtensionContext, view: SubagentReadModel) {
	...
	while (true) {
		...
		await ctx.ui.custom<null>(
			(tui, theme, keybindings, done) => new TakeoverView(tui, theme, keybindings, picked, view, done),
			{
				overlay: true,
				overlayOptions: { anchor: "center", width: "100%", maxHeight: "100%" },
			},
...
export function reconcileDashboardSelection(
	...
	selection.index =
		stableIndex >= 0 ? stableIndex : Math.min(Math.max(0, selection.index), Math.max(0, subs.length - 1));
...
class SubagentDashboard implements Component {
	...
	private renderRows(subs: ReadonlyArray<SubagentSnapshot>, width: number, height: number): string[] {
		...
		if (subs.length > height) {
			start = Math.min(Math.max(0, this.selection.index - Math.floor(height / 2)), subs.length - height);
		}
		...
		if (start + height < subs.length) {
			out[out.length - 1] = truncateToWidth(theme.fg("dim", `   ... ${subs.length - start - height} more`), width);
		}
		return out;
	}
