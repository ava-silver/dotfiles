...
export default function (pi: ExtensionAPI): void {
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (API_MERGE.test(command)) {
      return { block: true, reason: "PR merges through gh api are blocked. Do not bypass this guard; use gh pr merge from the current branch." };
    }
    if (!MERGE_COMMAND.test(command)) return;
    if (EXPLICIT_PR.test(command)) {
      return { block: true, reason: "Explicit PR numbers are blocked. Do not bypass this guard; merge only the current branch's PR." };
    }
    ...
    try {
      ...
      if (details.author?.login !== viewer || !details.headRefName?.startsWith("ava.silver/")) {
        return { block: true, reason: "Only the authenticated user's ava.silver/* branch PR may be merged. Do not bypass this guard." };
      }
    } catch (error) {
      return { block: true, reason: `Could not verify the current branch PR: ${error instanceof Error ? error.message : String(error)}. Do not bypass this guard.` };
    }
  });
}
