...
test("clamps over-capacity and nonsensical token values", () => {
	assert.equal(contextPercent({ tokens: 500_000, contextWindow: 200_000 }), 100);
	assert.equal(
	...
	assert.equal(formatContextUtilization({ tokens: -1, contextWindow: 200_000 }), "?%/200k");
});

test("handles missing usage or capacity without NaN or Infinity", () => {
	assert.equal(formatContextUtilization({ tokens: null, contextWindow: 372_000 }), "?%/372k");
	assert.equal(formatContextUtilization({ tokens: 12_000 }), "");
	assert.equal(formatContextUtilization({ tokens: 12_000, contextWindow: 0 }), "");
});
