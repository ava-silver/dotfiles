...
export class RunController {
	...
	schedule<T>(task: (signal: AbortSignal) => Promise<T>, invocationSignal?: AbortSignal): Promise<T> {
		...
		if (this.callCount >= MAX_AGENT_CALLS) {
			return Promise.reject(new Error(`Workflow exceeded the limit of ${MAX_AGENT_CALLS} agent calls`));
		}
		this.callCount++;
