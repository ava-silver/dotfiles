...
async function connectSlack(
  ...
  const result = await authenticate({
    ...
    onAuthorizationUrl: (url) =>
      ctx.ui.notify(`If no browser opened, open: ${url}`, "info"),
  });
...
export default function slackMcpExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    description:
      "Connect Pi's Slack MCP server. Opens a browser for the user to approve Slack access.",
