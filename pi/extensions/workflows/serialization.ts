...
export function toSerializable(value: unknown, options: SerializationOptions = {}): unknown {
	...
	const visit = (current: unknown, depth: number, location: string): unknown => {
		...
		for (const key of keys) {
			try {
				result[key] = visit((current as Record<string, unknown>)[key], depth + 1, `${location}.${key}`);
			} catch (error) {
				result[key] = `[unreadable property: ${error instanceof Error ? error.message : String(error)}]`;
			}
		}
		return result;
	};
